...
class AssessmentSessionPersistenceTest extends TestCase
{
    ...
    public function test_student_starts_saves_and_resumes_one_owned_session(): void
    {
        ...
        $this->patchJson("/api/v1/student/assessments/riasec/sessions/{$created['id']}", [
            'answers' => ['1' => 1, '2' => 2],
            'current_question' => 3,
        ])
            ->assertOk()
        ...
        $this->getJson('/api/v1/student/assessments/riasec/session')
            ->assertOk()
            ->assertJsonPath('data.id', $created['id'])
            ->assertJsonPath('data.answers.1', 1)
            ->assertJsonPath('data.current_question', 3);
    ...
    public function test_student_cannot_read_or_change_another_students_session(): void
    {
        ...
        $this->actingAs($other)
            ->patchJson("/api/v1/student/assessments/riasec/sessions/{$session->getKey()}", [
                'answers' => ['1' => 1],
                'current_question' => 2,
            ])
            ->assertNotFound();
    ...
    public function test_complete_session_ignores_late_save_replays_without_changing_submitted_answers(): void
    {
        ...
        $this->actingAs($student)
            ->postJson("/api/v1/student/assessments/riasec/sessions/{$session->getKey()}/submit")
            ->assertOk()
            ->assertJsonPath('data.status', 'result_available')
            ->assertJsonCount(6, 'data.result.result');

        $this->patchJson("/api/v1/student/assessments/riasec/sessions/{$session->getKey()}", [
            'answers' => $answers,
            'current_question' => 42,
        ])
            ->assertOk()
        ...
        $this->patchJson("/api/v1/student/assessments/riasec/sessions/{$session->getKey()}", [
            ...
            ->assertOk()
            ->assertJsonPath('data.answers.1', 1)
            ->assertJsonPath('data.current_question', 42)
            ->assertJsonPath('meta.save_ignored', true);
    ...
    public function test_result_processor_makes_the_result_available(): void
    {
        ...
        (new ProcessAssessmentResult($session->getKey()))
            ->handle(app(RiasecQuestionnaire::class));
    ...
    public function test_loading_a_preparing_session_finishes_the_result_immediately(): void
    {
        $student = $this->student();
        ...
        $this->actingAs($student)
            ->getJson('/api/v1/student/assessments/riasec/session')
            ->assertOk()
            ->assertJsonPath('data.status', 'result_available')
            ->assertJsonCount(6, 'data.result.result');
    }
    ...
    public function test_failed_processing_is_recoverable_without_unlocking_answers(): void
    {
        ...
        $job->failed(new RuntimeException('Processing details must not be stored.'));

        $session->refresh();
        $this->assertSame('result_failed', $session->status);
        $this->assertSame('ASSESSMENT_PROCESSING_FAILED', $session->processing_error_code);
        $this->assertNotNull($session->processing_failed_at);

        $this->actingAs($student)
            ->postJson("/api/v1/student/assessments/riasec/sessions/{$session->getKey()}/retry-result")
            ->assertOk()
            ->assertJsonPath('data.status', 'result_available')
            ->assertJsonPath('data.answer_count', 42);

        $this->assertDatabaseMissing('assessment_sessions', [
            'id' => $session->getKey(),
            'processing_error_code' => 'Processing details must not be stored.',
        ]);
    }

    public function test_current_session_normalizes_a_previously_saved_legacy_result(): void
    {
        $student = $this->student();
        AssessmentSession::query()->create([
            'user_id' => $student->getKey(),
    ...
    public function test_completed_attempt_is_immutable_and_a_retake_creates_linked_history_immediately(): void
    {
        ...
        $this->getJson('/api/v1/student/assessments/riasec/history')
            ->assertOk()
    ...
    public function test_retake_reason_is_optional_validated_and_cannot_change_after_creation(): void
    {
        ...
        $second = $this->postJson('/api/v1/student/assessments/riasec/sessions')
            ->assertCreated()
            ->assertJsonPath('data.retake_reason', null)
            ->json('data');

        $this->patchJson("/api/v1/student/assessments/riasec/sessions/{$second['id']}", [
            'answers' => [1 => 1],
            'current_question' => 1,
