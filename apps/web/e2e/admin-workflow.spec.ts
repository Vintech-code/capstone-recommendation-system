...
async function installAdminApi(page: Page) {
  ...
  await page.route('**/api/v1/**', (route) => {
    ...
    if (path === '/api/v1/admin/reports') return json(route, { data: { generatedAt: '2026-08-08T12:00:00+08:00', from: null, to: null, studentCount: 1, completedAssessments: 1, recommendationRuns: 1, minimumGroupSize: 5, suppressedCount: 1, topMatchFrequency: [] } })
...
test('Admin guidance workspace is responsive, accessible, and navigable', async ({ page }, testInfo) => {
  ...
  for (const [path, heading] of [
    ['/admin/assessments', 'Assessment activity'],
