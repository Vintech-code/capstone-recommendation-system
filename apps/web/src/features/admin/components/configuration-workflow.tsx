...
function ConfigurationWorkflow({ programmeId, onPublished }: { programmeId?: string; onPublished?: () => void }) {
  const resource = useAdminResource<ConfigurationWorkspace>('/configurations/catalogue')
  ...
  async function createDraft(sourceVersionId?: number) {
    ...
    try {
      await mutateAdmin('/configurations/catalogue', 'POST', sourceVersionId ? { sourceVersionId } : {})
  ...
  return <section className="border-y border-border bg-background py-5" aria-labelledby="catalogue-governance-heading">
    <div className="flex flex-col gap-3 px-1 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Controlled programme update</p><h2 id="catalogue-governance-heading" className="mt-1 font-display text-xl font-semibold">Student enrichment and media</h2><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Official programme facts and recommendation profiles are protected. Editable fields are limited to clearly labelled guidance, external career mappings, and media.</p></div>{!draft ? <Button type="button" className="rounded-xs" disabled={busy} onClick={() => void createDraft()}><GitBranch aria-hidden="true" /> Start a catalogue draft</Button> : null}</div>
...
function ConfigurationEditor({ draft, baseline, programmeId, onChanged, onPublished }: { draft: ConfigurationVersion; baseline: Record<string, unknown>; programmeId?: string; onChanged: () => void; onPublished?: () => void }) {
  ...
  return <div className="mt-6 border-t border-border pt-5">
    ...
    <CatalogueProfileEditor payload={payload} programmeId={programmeId} onChange={changePayload} />
