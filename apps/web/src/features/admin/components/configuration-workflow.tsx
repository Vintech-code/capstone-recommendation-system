...
function ConfigurationEditor({ draft, baseline, programmeId, onChanged, onPublished }: { draft: ConfigurationVersion; baseline: Record<string, unknown>; programmeId?: string; onChanged: () => void; onPublished?: () => void }) {
  ...
  return <div className="mt-6 bg-secondary p-4">
    ...
    <p className={`mt-2 text-xs font-semibold ${autosaveStatus === 'failed' ? 'text-destructive-ink' : 'text-muted-foreground'}`} role="status">{autosaveStatus === 'saved' ? 'Draft autosaved' : autosaveStatus === 'pending' ? 'Autosave pending…' : autosaveStatus === 'saving' ? 'Saving draft…' : 'Autosave failed — use Save for later to retry.'}</p>
