...
> = {
  ...
  A: {
    name: "Artistic",
    description:
      "Creative problem solving, visual expression, unconventional ideation",
    color: "var(--riasec-a)",
  ...
  S: {
    name: "Social",
    description:
      "Interpersonal communication, collaborative learning, student support",
    color: "var(--riasec-s)",
  ...
  E: {
    name: "Enterprising",
    description:
      "Initiative taking, organizational leadership, persuasive discourse",
    color: "var(--riasec-e)",
  ...
  return (
    <div className="space-y-5 pb-10" data-report-print>
      ...
      {latestAttempt ? (
        <>
          <div
            data-testid="student-resume-card"
            className="overflow-hidden rounded-xs border border-border bg-card shadow-sm"
          >
              ...
              <div className="relative z-10 flex flex-col items-start justify-between gap-4 lg:flex-row">
                ...
                <div className="flex items-start sm:items-center gap-5 flex-wrap sm:flex-nowrap">
                  ...
                  <div className="flex flex-col gap-1.5 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 font-body text-xs sm:text-sm">
                      ...
                      {student.profile?.lrn ? (
                        <span className="font-semibold text-primary-ink">
                          LRN: {student.profile.lrn}
                        </span>
                      ) : null}
                      {student.profile?.age != null ? (
                        <span className="text-muted-foreground">
                          {student.profile.age} years old
                        </span>
                      ) : null}
                      {latestAttempt.entranceExamination ? (
                        <span className="font-semibold text-primary-ink">
                          Entrance score:{" "}
                          {latestAttempt.entranceExamination.score} (
                          {latestAttempt.entranceExamination
                    ...
                    {student.profile?.shsSchoolName ||
                    student.profile?.shsStrand ? (
                      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground sm:text-sm">
                        ...
                        {student.profile.shsStrand ? (
                          <span>
                            {student.profile.shsStrand}
                            {student.profile.shsGraduationYear
                              ? ` · Class of ${student.profile.shsGraduationYear}`
                              : ""}
                          </span>
                        ) : null}
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 font-body text-xs sm:text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {student.attempts.length} assessment attempt
                        {student.attempts.length === 1 ? "" : "s"}
                      </span>
                      <span className="text-muted-foreground">
                        {student.savedProgrammeCount} saved programme
                        {student.savedProgrammeCount === 1 ? "" : "s"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex h-full flex-col items-center justify-between rounded-xs border border-border bg-muted/20 p-4 lg:col-span-5 sm:p-5">
                  <div className="w-full flex items-center justify-between gap-2 mb-1">
                    <div>
                      ...
                      <p className="font-body text-[11px] text-muted-foreground">
                        Visual score distribution across the 6 RIASEC interest
                        areas
                      </p>
                    </div>
                  ...
                  <div className="mt-1 w-full space-y-2.5 rounded-xs border border-border bg-card p-3 text-left shadow-sm">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      ...
                      . Outer peaks on the hexagon highlight areas of strongest
                      intrinsic enthusiasm, focused aptitude, and curriculum
                      readiness.
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-0.5 font-label text-[11px]">
                      ...
                      <div className="rounded-xs border border-border/50 bg-muted/40 p-2">
                        ...
                        <span className="font-bold text-foreground truncate block mt-0.5">
                          {primaryCodes.includes("I") &&
                          primaryCodes.includes("C")
              ...
              {(latestAttempt.recommendations ?? [])
                ...
                .map((programme, idx) => {
                  const priorityBadge =
                    idx === 0
                      ? {
                          label: "Recommended Priority 1",
                          tone: "border-primary/25 bg-primary/15 text-primary-ink",
                        }
                      : idx === 1
                        ? {
                            label: "Viable Secondary Pathway",
                            tone: "border-info/25 bg-info/15 text-info-foreground",
                          }
                        : {
                            label: "Exploratory Option",
                            tone: "bg-amber-500/15 text-amber-800 dark:text-amber-200 border-amber-500/25",
                          };

                  const college =
                    programme.code.includes("IT") ||
                    programme.name.includes("Information Technology")
                      ? "College of Computer & Information Sciences"
                      : programme.code.includes("BA") ||
                          programme.name.includes("Business")
                      ...
                      <div className="space-y-3 pt-3 border-t border-border">
                        ...
                        <div className="flex justify-between font-label text-xs">
                          ...
                          <span className="font-bold text-foreground">
                            {latestAttempt.entranceExamination?.score ?? "1.50"}{" "}
                            (Eligible)
                          </span>
                        </div>
...
function RiasecRadarSvg({
  ...
  const ptI = {
    x: Math.round(150 + 104 * iRatio),
    y: Math.round(150 - 60 * iRatio),
    code: "I",
  };
  const ptA = {
    x: Math.round(150 + 104 * aRatio),
    y: Math.round(150 + 60 * aRatio),
    code: "A",
  };
  const ptS = { x: 150, y: Math.round(150 + 120 * sRatio), code: "S" };
  const ptE = {
    x: Math.round(150 - 104 * eRatio),
    y: Math.round(150 + 60 * eRatio),
    code: "E",
  };
  const ptC = {
    x: Math.round(150 - 104 * cRatio),
    y: Math.round(150 - 60 * cRatio),
    code: "C",
  };
