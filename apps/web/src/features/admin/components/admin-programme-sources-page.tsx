...
function AdminProgrammeSourcesPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return <div className="space-y-5">
    ...
    {resource.data.length ? <section className="overflow-hidden rounded-3xl border border-border bg-card shadow-[var(--shadow-card)]" aria-labelledby="source-registry-heading">
...
function SourceReviewBadge({ status }: { status: ProgrammeSourceRegistryEntry['reviewStatus'] }) {
  ...
  if (status === 'review_due') return <span className="inline-flex items-center gap-1 rounded-full bg-canvas-cream px-2.5 py-1 text-[11px] font-semibold text-warning"><Clock3 aria-hidden="true" className="size-3.5" />Review due</span>
...
function SourceSummary({ label, value, tone }: { label: string; value: number; tone: 'blue' | 'green' | 'orange' }) {
  const tones = { blue: 'bg-primary-fixed text-on-primary-fixed', green: 'bg-success/10 text-success', orange: 'bg-canvas-cream text-warning' }
