...
describe('Student recommendation results', () => {
  it('renders only recommendation data supplied by the API boundary', () => {
    ...
    const resultHeading = screen.getByRole('heading', { level: 1, name: 'Investigative and Conventional' })
    expect(resultHeading).toBeVisible()
    expect(resultHeading).toHaveClass('font-display', 'font-black')
    expect(resultHeading.closest('section')).not.toHaveClass('border', 'bg-card', 'shadow-[var(--shadow-card)]')
    expect(screen.queryByRole('img', { name: /RIASEC profile/i })).not.toBeInTheDocument()
    const resultSummary = screen.getByText(/investigative and conventional pursuits/i)
    ...
    const careerHeading = screen.getByRole('heading', { name: 'Recommended career paths' })
    ...
    expect(careerSection).not.toHaveClass('rounded-2xl', 'bg-card', 'shadow-sm')
    expect(screen.getByRole('heading', { name: 'RIASEC scores' })).toHaveClass('text-2xl', 'font-extrabold')
