...
function StudentRecommendationDetailPage({
  ...
  const circleOffset =
    circleLength * (1 - Math.min(Math.max(course.match, 0), 100) / 100);
  ...
  const coverStyle = course.coverImageUrl
    ? programmeMediaStyle(course.coverImagePosition)
    : undefined;
  const logoStyle = course.logoImageUrl
    ? programmeMediaStyle(course.logoImagePosition)
    : undefined;
  ...
  const programmeTypeLabel =
    course.eligibilityGroup === "non_board"
      ? "Non-board programme"
      : "Board programme";
  ...
  const facts = [
    {
      label: "Duration",
      value: course.duration || "Not published",
      accent: false,
      icon: Clock3,
    },
    {
      label: "Degree type",
      value: course.degreeType || "Not published",
      accent: false,
      icon: GraduationCap,
    },
    {
      label: "Programme type",
      value: programmeTypeLabel,
      accent: true,
      icon: ShieldCheck,
    },
    {
      ...
      value:
        course.careerDirections.length > 0
          ? `${course.careerDirections.length} to explore`
          : "Not configured",
      accent: false,
  ...
  return (
    <article className="pb-16" aria-labelledby="course-detail-title">
      <header className="relative isolate min-h-[25rem] overflow-hidden bg-primary-fixed/45">
        ...
        <div
          aria-hidden="true"
          className="absolute inset-0 -z-10 bg-gradient-to-r from-background/90 via-background/55 to-transparent"
        />
        <div
          aria-hidden="true"
          className="absolute inset-0 -z-10 bg-gradient-to-t from-background via-background/25 to-transparent"
        />
        <div className="student-page relative z-10 flex min-h-[25rem] items-center py-12 sm:py-16">
          {logo ? (
            <img
              ...
              style={logoStyle}
              className={`absolute right-5 top-8 size-20 rounded-xl bg-white/90 p-2 shadow-sm sm:right-8 sm:size-28 lg:right-10 lg:top-12 lg:size-36 ${logoStyle ? "object-cover" : "object-contain"}`}
            />
          ) : null}
            ...
            <div className="mt-5 grid gap-7 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
              <div className="max-w-4xl pr-0 sm:pr-32 lg:pr-0">
                <h1
                  ...
                  className="font-display text-4xl font-bold tracking-[-0.04em] sm:text-5xl lg:text-6xl"
                >
                  {course.name}
                </h1>
                {course.summary ? (
      ...
      <div className="student-page mt-8 grid items-start gap-10 lg:grid-cols-[minmax(0,1.6fr)_minmax(18rem,0.72fr)]">
        <div className="space-y-8">
          <dl className="grid grid-cols-2 gap-4 border-b border-border/80 pb-6 sm:grid-cols-4 sm:gap-6">
            {facts.map((fact) => (
              <div key={fact.label} className="min-w-0">
                ...
                <dd className="mt-1.5">
                  <span
                    className={`block font-display text-base font-bold sm:text-lg ${fact.accent ? "text-primary-ink" : "text-foreground"}`}
                  >
                    {fact.value}
                  </span>
                </dd>
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              />
              <div>
                <p className="font-label text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">
                  Programme overview
                </p>
                <h2
            ...
            {course.learningAreas.length > 0 ? (
              <ol className="overflow-hidden rounded-2xl border border-border/70 bg-card/60 shadow-sm divide-y divide-border/60">
                {course.learningAreas.map((area, index) => (
                  <li
                    key={area}
                    className="flex items-start gap-4 p-4.5 sm:p-5 transition-colors hover:bg-card"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs leading-relaxed text-muted-foreground sm:text-sm">
                        {course.learningAreaDescriptions?.[area] ??
                          "A recorded learning area in the current programme catalogue."}
                      </p>
                    </div>
                  </li>
                ))}
          ...
          <section aria-labelledby="fit-reasons-title" className="space-y-3.5">
            ...
            {fitReasons.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">
                No additional match factors were supplied for this
                recommendation.
              </p>
            )}
          </section>
        ...
        <aside className="space-y-6">
          ...
          >
            ...
            <p className="mt-3 text-center text-sm font-semibold text-primary-ink">
              {fitLabel}
            </p>
            <p className="mt-2 text-center text-xs leading-5 text-muted-foreground sm:text-sm">
              Based on the interest profile recorded in your completed
              assessment.
            </p>
            <div
              role="progressbar"
              aria-label={`${course.name} ${fitLabel.toLowerCase()}`}
              aria-valuemin={0}
              aria-valuemax={100}
            ...
            <div className="mt-6 border-t border-border pt-5">
              <dl className="space-y-4">
                <div>
                  <dt className="font-label text-xs uppercase tracking-[0.1em] text-muted-foreground">
                    Programme code
                  </dt>
          ...
          <CareerDirectionsSection
            directions={course.careerDirections}
            opportunities={course.careerOpportunities}
          />
        </aside>
      </div>
    </article>
