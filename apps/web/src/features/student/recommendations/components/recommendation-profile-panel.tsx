...
function RecommendationProfilePanel({
  result,
  className,
}: RecommendationProfilePanelProps) {
  if (!result) {
    return (
      <aside
      ...
      >
        <h2 className="font-display text-xl font-bold text-foreground">
          RIASEC scores unavailable
        </h2>
        <p className="mt-2 text-base font-medium leading-7 text-muted-foreground">
          Your programme matches are available, but the six assessment values
          could not be loaded here.
        </p>
      </aside>
    );
  }
  ...
  return (
    ...
    >
      <div>
        <h2
          id="profile-breakdown-title"
          className="font-display text-2xl font-extrabold tracking-tight text-foreground sm:text-3xl"
        >
          RIASEC scores
        </h2>
        <p className="mt-1.5 text-sm font-medium leading-relaxed text-muted-foreground sm:text-base">
            ...
            <div key={dimension.code}>
              <div className="flex items-center justify-between gap-3 font-label text-sm sm:text-base">
                <span className="flex min-w-0 items-center gap-2 font-semibold">
                  <span
                    ...
                    style={{
                      backgroundColor:
                        dimensionColors[dimension.code] ?? "var(--primary)",
                    }}
                  />
              ...
              <div
                ...
                aria-valuenow={Math.max(
                  minimum,
                  Math.min(maximum, dimension.value),
                )}
