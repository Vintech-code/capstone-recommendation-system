...
describe("StudentWorkspaceShell", () => {
  it("uses one compact non-sticky mobile header row with a transparent logo treatment", () => {
    ...
    const logos = container.querySelectorAll(
      'button[aria-label="Go to dashboard"] img',
    );
    ...
    expect(mobileNavigation.querySelector("ul")).toHaveClass("grid-cols-3");
    expect(mobileNavigation.parentElement).toHaveClass("flex", "h-16");
    expect(
      screen.queryByRole("button", { name: "Dashboard" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getAllByRole("button", { name: "My Matches" })[1],
    ).toHaveAttribute("aria-current", "page");
  });

  it("uses different catalogue and match icons", () => {
    const programmesIcon = dashboards.student.modules.find(
      (module) => module.id === "programmes",
    )?.icon;
    const matchesIcon = dashboards.student.modules.find(
      (module) => module.id === "recommendations",
    )?.icon;
  ...
  it("keeps mobile navigation functional", () => {
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: "Explore Programs" })[1],
    );
  ...
  it("does not select a navigation tab when viewing off-navigation views like history", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Assessment history" }),
    ).not.toBeInTheDocument();
    expect(
