...
describe("Student dashboard", () => {
  beforeEach(() => {
    vi.mocked(fetch).mockImplementation(async (input) => {
      ...
      if (url === "/api/v1/student/recommendations/latest") {
        return Response.json({
          data: { status: "not_available", recommendation: null },
        });
      }
            ...
            programmes: [
              {
                ...
                requirements: ["Meet published admission requirements."],
      ...
      if (url === "/api/v1/student/assessments/riasec/history") {
        return Response.json({
          data: [
            { ...testAssessmentLifecycle, attempt_number: 1, is_current: true },
          ],
          policy: {
            status: "proposed",
            version: "RETAKE-PROPOSED-2026-01",
            minimum_days_between_completed_attempts: 0,
      ...
      if (url === "/api/v1/student/recommendations/attempts/1") {
        return Response.json({
          data: {
            status: "available",
            recommendation: testRecommendationSnapshot,
          },
        });
      }
  ...
  it.skip("shows the assessment and recommendation summary without a report module", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: /Your strongest interests are/,
      }),
    ).toBeVisible();
    ...
    expect(screen.getByTestId("student-dashboard-summary")).not.toHaveClass(
      "max-w-[96rem]",
    );
  ...
  it.skip("opens the next recommended-course step from the dashboard", async () => {
    ...
    await user.click(
      await screen.findByRole("button", {
        name: "Explore all recommendations",
      }),
    );
  ...
  it("uses the clean journey overview without a redundant completed-assessment card", async () => {
    ...
    render(
      <StudentDashboardPage
        ...
        initialLifecycle={testAssessmentLifecycle}
    ...
    expect(
      screen.getByText(
        "Explore programmes connected to Investigative, Conventional, and Social interests, then compare your strongest matches.",
      ),
    ).toBeVisible();
    expect(
      screen.getByRole("button", { name: "Explore your matches" }),
    ).toHaveClass("rounded-xl");
    expect(
      screen.getByRole("heading", { name: "Keep moving forward" }),
    ).toBeVisible();
    expect(
      screen.getByRole("progressbar", { name: "Academic journey progress" }),
    ).toHaveAttribute("aria-valuenow", "100");
    expect(
      screen.queryByRole("heading", { name: "Your result is available" }),
    ).not.toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByText(
        "Your highest recorded areas are Investigative, Conventional, and Social.",
      ),
    ).toBeVisible();
    expect(
      screen.getByLabelText("Primary recorded interest"),
    ).toBeInTheDocument();
    ...
    expect(
      document.querySelector(".lucide-sparkles, .lucide-star"),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByRole("progressbar", { name: /course match/i }),
    ).toHaveAttribute("aria-valuenow", "90");
    expect(screen.getByText("Recommended programme")).toBeVisible();
    expect(
      screen.queryByText(/students on their journey/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /appointment/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("opens a completed attempt with its saved result and recommendations", async () => {
    ...
    expect(
      screen.getByText("Assessment version: test-instrument"),
    ).toBeVisible();
    expect(screen.getByText("Current result")).toBeVisible();
    expect(
      screen.getByText("Programme matches from this attempt"),
    ).toBeVisible();
    expect(await screen.findByText("Test Course")).toBeVisible();
    expect(fetch).toHaveBeenCalledWith(
      "/api/v1/student/recommendations/attempts/1",
      expect.any(Object),
    );
  });
  ...
  it("compares a completed retake with the previous completed attempt using recorded values only", async () => {
    ...
    vi.mocked(fetch).mockImplementation(async (input) => {
      ...
      if (url === "/api/v1/student/assessments/riasec/session")
        return Response.json({ data: secondAttempt });
      if (url === "/api/v1/student/recommendations/attempts/2")
        return Response.json({
          data: {
            status: "available",
            recommendation: testRecommendationSnapshot,
    ...
    expect(
      await screen.findByRole("heading", { name: "Compared with Attempt 1" }),
    ).toBeVisible();
    expect(
      screen.getByText("Assessment version: official-mini-ip-local-v1"),
    ).toBeVisible();
    expect(screen.getByText("Recorded score change only")).toBeVisible();
    expect(screen.getAllByText("+2")).toHaveLength(6);
    expect(
      screen.getByText(/I wanted to review my recorded interests/),
    ).toBeVisible();
  });
  ...
  it("preserves the latest completed result while a retake is in progress", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Your assessment timeline" }),
    ).toBeVisible();
  ...
  it("delegates to the selected attempt when clicking Explore Matches from history", async () => {
    const user = userEvent.setup();
    const onExploreMatches = vi.fn();
    vi.mocked(fetch).mockImplementation(async (input) => {
      const url = input.toString();
      if (url === "/api/v1/student/assessments/riasec/history") {
        return Response.json({
    ...
    const exploreButtons = await screen.findAllByRole("button", {
      name: "Explore Matches",
    });
    expect(exploreButtons.length).toBeGreaterThan(0);
    await user.click(exploreButtons[0]);
    expect(onExploreMatches).toHaveBeenCalledWith(1);
  });
  ...
  it("keeps the latest completed summary on the dashboard while a new result is preparing", async () => {
    vi.mocked(fetch).mockImplementation(async (input) => {
      ...
      if (url === "/api/v1/student/recommendations/latest") {
        return Response.json({
          data: { status: "preparing", recommendation: null },
        });
      }
      if (url === "/api/v1/student/assessments/riasec/history") {
        return Response.json({
    ...
    expect(
      await screen.findByRole("heading", {
        name: "Finalizing your submission",
      }),
    ).toBeVisible();
    expect(
      screen.getByRole("heading", {
  ...
  it("prioritizes a completed result over an empty retake session", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "View assessment result" }),
    ).toBeVisible();
    expect(
      screen.queryByRole("heading", { name: "Your result is available" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "View latest result" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("progressbar", { name: "Saved assessment progress" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Resume assessment" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("confirms a retake and displays a visible start failure", async () => {
    ...
    vi.mocked(fetch).mockImplementation(async (input) => {
      ...
      if (url === "/api/v1/student/assessments/riasec/sessions") {
        return Response.json(
          { message: "Unable to start the retake." },
          { status: 503 },
        );
      }
      return Response.json({ message: "Not found." }, { status: 404 });
    ...
    await user.click(
      await screen.findByRole("button", { name: "Start retake" }),
    );
