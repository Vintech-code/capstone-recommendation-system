...
describe("access portals and workspace shell", () => {
  it("opens the Student portal without exposing role selection", async () => {
    ...
    expect(
      screen.queryByRole("tablist", { name: "Authentication modes" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Gmail sign-in is not configured yet."),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByText("Course Recommendation System"),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("uses a separate portal and workspace for the Administrator role", async () => {
    ...
    // Administrators now also have the Google sign-in button (portal=admin path)
    expect(
      screen.getByRole("button", { name: "Continue with Google" }),
    ).toBeEnabled();
    // 'Create an account' is a student-only link; it must not appear on the admin portal
    expect(
    ...
    expect(
      screen.queryByRole("button", { name: "Assessments" }),
    ).not.toBeInTheDocument();
